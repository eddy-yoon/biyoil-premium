...
if edit_mode == "사진당 시간(속도) 조절":
    ...
    target_total_duration = st.sidebar.number_input("목표 전체 영상 길이(초)", 5, 60, 15)
    ...
    if files and logo:
        with st.spinner('모든 설정을 적용하여 영상을 굽는 중입니다...'):
            try:
                ...
                output = "biyoil_premium_custom.mp4"
                final_video.write_videofile(output, fps=24, codec="libx264", audio_codec="aac")
                st.video(output)
                st.success(f"🎉 제작 완료! (총 길이: {round(final_video.duration, 1)}초)")
            except Exception as e:
                st.error(f"제작 중 에러 발생: {str(e)}")
    else:
        st.warning("사진(영상)과 로고를 모두 올려주세요.")
